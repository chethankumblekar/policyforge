...
- **Kubernetes parser.** `internal/parser/k8s` flattens Pod-template workloads to a pod-security attribute shape.
- **Helm parser.** `internal/parser/helm` shells out to a locally installed `helm template` (vendoring Helm's own engine was rejected — it pulls in the full Kubernetes client-go/apimachinery stack just for offline rendering) and reuses `internal/parser/k8s`'s extraction logic on the result. `helm` must be on PATH to scan charts; its absence is a warning, not a scan failure.
