...
- [x] Enterprise module v1: self-hosted, network-gated dashboard (`enterprise/portal`) — SQLite persistence, Docker/Compose packaging, HTTP Basic Auth (both dashboard and `/api/scans`, with `--upload` embedding credentials as URL userinfo), wired to `policyforge scan --upload`. Hosting-model and licensing-mechanics questions from `enterprise/DESIGN.md` are now decided (self-hosted, network-gated); still open and not built: Entra ID SSO / per-user accounts, audit trail, compliance framework mapping (SOC2/PCI), SBOM/provenance ingestion — see DESIGN.md's remaining "Open questions"
